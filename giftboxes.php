...
where p.is_deleted=0 and p.status=1 and p.category_id=3 and p.country_id=".$_GET['country_id']." order by p.id desc limit {$showPostFrom},{$showPostCount}");
$html='';
...
where p.is_deleted=0 and p.status=1 and p.category_id=3 and p.country_id=".$country_id." order by p.id desc limit 0,8");

$totalRecord = $crud->number_of_records("SELECT id from products p where p.is_deleted=0 and p.status=1 and p.category_id=3 and p.country_id=".$country_id."");
...
<h3 class="font-size-14 ltr-sp-015 mt-50 mb-2"><em>What we have for you?</em></h3>
<h2 class="mt-0 mb-2">Gift Boxes</h2>
...
if(process) return false;
    ...
    function doSomething() {
       ...
       $.ajax({
                type: "POST",
                url: "giftboxes.php?fetch_prod=1&country_id="+country_id+"",              
