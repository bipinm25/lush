...
where p.is_deleted=0 and p.status=1 and p.category_id=5 and p.country_id=".$_GET['country_id']." order by p.id desc limit {$showPostFrom},{$showPostCount}");
$html='';
...
where p.is_deleted=0 and p.status=1 and p.category_id=5 and p.country_id=".$country_id." order by p.id desc limit 0,8");

$totalRecord = $crud->number_of_records("SELECT id from products p where p.is_deleted=0 and p.status=1 and p.category_id=5 and p.country_id=".$country_id."");
...
if(process) return false;
    ...
    function doSomething() {
       ...
       $.ajax({
                type: "POST",
                url: "flowers.php?fetch_prod=1&country_id="+country_id+"",              
